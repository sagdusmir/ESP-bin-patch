...
TL;DR: **patch secrets in an already-built `.bin`** (Wi-Fi SSID, Home Assistant API encryption key, …) without shifting offsets or leaving a checksum that the ROM bootloader will reject. However, there are some limitations.
...
Check out [`https://sagdusmir.github.io/ESP-bin-patcher/`](https://sagdusmir.github.io/ESP-bin-patcher/). This is a static page (with a JavaScript port of the patcher) that also allows **installing the patched image over USB**  using ESP Web Tools.
